...
class adatbazis{
	...
	public function insert_user_form(){
		...
		<form method="POST">
			...
			Add meg az árát: <br />
			<input type="email" name="imput_ár"><br />		
			Add meg az elérhetőségét: <br />	
			<select name="imput_elérhetöseg">
				<option value='1'>Elérhető</option>
				<option value='0' selected>Nincs raktáron</option>
			</select><br />	
		...
		if ($this->result->num_rows > 0) {
			...
			while($this->row = $this->result->fetch_assoc()) {
					...
					<form method="POST">
						...
						<input type="email" name="imput_ár"
	...
	public function cmd_update_user(){
		$this->sql = "UPDATE 
						users
					  SET
						`user_username`='".$_POST["input_name"]."',
						`user_email`='".$_POST["imput_ár"]."' 
					  WHERE
					     `user_id` = ". $_POST["input_id"]."
							;";
